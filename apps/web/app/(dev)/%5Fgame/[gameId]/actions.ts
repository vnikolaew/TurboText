...
const USERS_READY_QUEUE: Map<string, Set<string>> = new Map<string, Set<string>>();
const USERS_FINISHED_SETS: Map<string, Set<string>> = new Map<string, Set<string>>();
...
export const finishChallenge = authorizedAction
   ...
   .action(async ({ ctx: { userId }, parsedInput: { gameId, ...rest } }) => {
      ...
      let challenge = await xprisma.usersChallenge.findUnique({ where: { id: gameId } });
      ...
      if (USERS_FINISHED_SETS.get(gameId)?.size === 2) {
         ...
         challenge = await xprisma.usersChallenge.update({
            ...
            data: {
               ...
               ...(challenge.userOneId === userId && { userOneRunId: result.run!.id }),
               ...(challenge.userTwoId === userId && { userTwoRunId: result.run!.id }),
            },
         });
