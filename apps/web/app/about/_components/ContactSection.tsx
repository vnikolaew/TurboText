...
const ContactSection = ({}: ContactSectionProps) => {
   return (
      <section id={`contact`} className={`text-left flex flex-col items-start gap-4 w-full !mb-12`}>
         ...
         <div className={`w-full flex items-center gap-4 h-20 mt-4`}>
            <ContactLink href={`mailto:${LINKS.email}`} text={`Mail`} icon={<Mail size={24} />} />
            <ContactLink href={`${LINKS.twitter}`} text={`Twitter`} icon={<Twitter size={24} />} />
            <ContactLink href={LINKS.discord} text={`Discord`} icon={
               <svg width="800px" height="800px" viewBox="0 -28.5 256 256" version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    preserveAspectRatio="xMidYMid" className={`w-6 h-6`}>
                  <g>
                     <path
            ...
            <ContactLink href={LINKS.github} text={`Github`} icon={
               <Github size={24} />
            } />
         </div>
      </section>
