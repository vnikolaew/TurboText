   ...
   return (
      <section id={`stats`} className={`flex flex-col items-center gap-4 w-full`}>
         ...
         <div className={`grid grid-cols-3 w-full gap-8 mt-8`}>
            ...
            <TypingRunStat valueClassName={`text-accent`} label={`average wpm`} value={averageWpm.toFixed(0)} />
            ...
            <TypingRunStat valueClassName={`text-accent`} label={`average raw wpm`} value={averageRawWpm.toFixed(0)} />
            ...
            <TypingRunStat valueClassName={`text-accent`} label={`average accuracy`} value={`${averageAcc.toFixed(0)}%`} />
            ...
            <TypingRunStat valueClassName={`text-accent`} label={`average consistency`} value={`${averageConsistency.toFixed(0)}%`} />
            <TypingRunStat label={`average consistency (last 10 tests)`}
...
const TypingRunStat = ({ label, value, valueClassName }: {
   label: ReactNode;
   value: ReactNode,
   ...
   return (
      <div className={`flex flex-col items-start gap-2`}>
         ...
         <span className={cn(`text-5xl text-main`, valueClassName)}>{value}</span>
      </div>
   );
