...
const Page = async ({ params: { gameId } }: PageProps) => {
   ...
   return match(challenge)
      ...
      .when(c => c.state === UsersChallengeState.Finished, async _ => {
      ...
      .when(challenge => ![challenge.userOneId, challenge.userTwoId].includes(session?.user?.id) ||
         challenge?.state === UsersChallengeState.Stopped, challenge => {
         return <section
            className={`mx-auto my-24 flex w-2/3 flex-col items-center gap-4`}
         >
            <div className={`flex flex-col items-center text-xl`}>
