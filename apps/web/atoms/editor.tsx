...
export const typingModeAtom = atomWithStorage<TypingMode>(
   `mode`,
   TypingMode.TIME
);
...
export const wordRangesByEndsAtom = atom<Map<number, [number, number]>>(
   (get) => {
      const wordRanges = get(wordRangesAtom);
      return new Map([
         ...wordRanges.map(
            ({ range: [start, end] }) => [end, [start, end] as const] as const
         ),
      ]);
   }
);
...
export const wordsCorrectnessAtom = atom<(boolean | null)[]>((get) => {
   ...
   return wordRanges
      ...
      .map(({ range: [start, end] }) =>
         lettersCorrectness.slice(start, end + 1).every((x) => x === true)
      );
});
...
export const lettersCorrectnessPercentageAtom = atom<number>((get) => {
   ...
   return (
      (lettersCorrectness.filter(Boolean)?.length /
         lettersCorrectness.filter((l) => l !== null)?.length) *
      100
   );
});
...
export const completedWordsAtom = atom((get) => {
   ...
   return wordRanges.filter(({ range: [, end] }) => {
...
export const wordsCompletionTimesAtom = atom((get) => {
   ...
   return completedWords
      ...
      ?.map(({ range: [start, end], word }) => {
         const time =
            lettersReversed.find((l) => l.charIndex === end)?.timestamp! -
...
export const totalRunTimeAtom = atom<number>((get) => {
   ...
   return (
      (endTime === 0 ? performance.now() : endTime) - start - totalPauseTime
   );
});
...
export const typingRunSuccessAtom = atom<TypingRunSuccess>((get, _) => {
   ...
   const userTestDifficulty: string = get(
      userTestDifficultyAtom
   ...
   if (
      userTestDifficulty === `MASTER` &&
      lettersCorrectness.some((x) => x === false)
   ) {
   ...
   if (
      userTestDifficulty === `EXPERT` &&
      wordsCorrectness.some((l) => l === false)
   ) {
      console.log(`Test failed. Level: ${userTestDifficulty}`);
   ...
   if (runState === TypingRunState.RUNNING)
   ...
   if (runState === TypingRunState.FINISHED) {
      ...
      if (
         userTestDifficulty === `MASTER` &&
         lettersCorrectness.some((l) => l === false)
      )
         return TypingRunSuccess.FAILED;
...
export const typingRunAtom = atom<TypingRun>(
   (get) => {
      const typedLetters = get(typedLettersAtom);
      ...
      const wordCompletionsTimesTotal = sum(
         get(wordsCompletionTimesAtom).map((w) => w.time)
      );
      ...
      return {
         totalRunTime:
            totalRunTime === 0 ? wordCompletionsTimesTotal : totalRunTime,
         typedLetters,
...
export const onKeyPressAtom = atom(
   null,
   (get, set, e: KeyboardEvent<HTMLDivElement>) => {
      e.preventDefault();
      e.stopPropagation();
      e.cancelBubble = true;
      ...
      const confidenceMode = get(confidenceModeAtom) as unknown as
         | "OFF"
         | "ON"
         | "MAX";
      ...
      if (e.key === `Backspace`) {
         ...
         if (freedomMode || confidenceMode === "ON") {
            ...
            const isBackspacingOnPreviousWord = [
               ...wordRangesByEnds.keys(),
      ...
      if (charCode >= 32 && charCode <= "z".charCodeAt(0)) {
         ...
         const correct =
            charCode === charsByIndex[currentCharIndex + 1]?.charCodeAt(0);
...
export const missedWordsAtom = atom<string[]>((get) => {
   ...
   const missedWords = wordRanges
      .map(({ word, range: [start, end] }) => {
         const missedChars = sum(
            range(start, end + 1).map((charIndex) => {
               let i = [...typingRun.typedLetters.toReversed()]
                  // @ts-ignore
                  .findIndex(
                     (l) =>
                        l.charIndex === charIndex &&
...
export const slowWordsAtom = atom<string[]>((get) => {
   ...
   const wordTimesToFinish = wordRanges.map(({ word, range: [start, end] }) => {
      const startIndex = [...typingRun.typedLetters]
         .reverse()
         .findIndex((l) => l.charIndex === start);
      const endIndex = [...typingRun.typedLetters]
         .reverse()
         .findIndex((l) => l.charIndex === end);

      const time = Math.abs(
         (typingRun.typedLetters.at(endIndex)?.timestamp ?? 0) -
            (typingRun.typedLetters.at(startIndex)?.timestamp || 0)
      );
      return {
         time,
   ...
   let slowestWords = sortBy(
      wordTimesToFinish,
      (value, index) => -value.averageTimeForLetter
   )
      .slice(0, 5)
      .map((w) => w);
