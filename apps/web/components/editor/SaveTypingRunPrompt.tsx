...
const SaveTypingRunPrompt = ({ onSave, onDismiss, loading }: SaveTypingRunPromptProps) => {
   return (
      <MotionAlert
         ...
         exit={{ opacity: 0 }}
         key={`alert`} className={`flex !items-center animate-alert gap-4 `}>
         <div>
            <RocketIcon className="h-6 w-6 stroke-amber-600 shadow-md" />
         </div>
         ...
         <div className={`flex-1 flex justify-end items-center gap-4`}>
            <Button
               disabled={loading}
               onClick={_ => onSave?.()}
               className={`!px-8 rounded-lg`} variant={`default`}>Yes</Button>
            <Button
               disabled={loading}
               onClick={_ => onDismiss?.()}
