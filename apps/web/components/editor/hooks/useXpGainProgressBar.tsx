...
export function useXpGainProgressBar() {
   ...
   const currentLevel = useMemo(() => Math.ceil(Math.pow((userXp.points / 100), 1 / EXPONENT)), [userXp])
   ...
   useEffect(() => {
      ...
      if (userXp?.points > userXpPrev?.points) {
         ...
         hideTimeoutId = setTimeout(() => setShowProgressAnimation(false), 4000);
         changePercentageValue = setTimeout(() => setProgressValue(percentXp), 1500);
      }
   ...
   return { progressValue, showProgressAnimation, xpGained, percentsUntilNextLevel, currentLevel };
}